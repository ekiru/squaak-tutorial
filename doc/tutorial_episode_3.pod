...
# $Id: tutorial_episode_3.pod 36833 2009-02-17 20:09:26Z allison $
...
Finally, remove the rules C<proto token statement_control>, C<rule statement_control:sym<say> >, <rule statement_control:sym<print> >, and everything after the "## Operators" line.
...
The last two rules are the C<term:sym<integer_constant> > and C<term:sym<string_constant> > tokens. Their names have a different form from our other rules. They are specializations of the C<proto> token term. A proto rule is equivalent to a normal rule whose body consists of each specialization of the rule separated by the C<|> operator. The name of a particular specialization of a proto rule is placed between the angle brackets. Within the body of the rule, it can be matched literally with C<<sym>>.
